...
import { eq, sql } from "drizzle-orm"; // eq aur sql import karna zaruri hai
...
export async function approveAssetAction(assetId : string) {
  ...
  try {
    ...
    return {
      success: false,
      message: "Failed to approve users data",
    };
  }
}
export async function rejectAssetAction(assetId : string) {
  ...
  try {
    ...
    return {
      success: false,
     message: "Failed to reject users data",
    };
  }
}


export async function getPendingAssestsAction() {
    const session = await auth.api.getSession({
    headers: await headers(),
  });

  if (!session?.user || session.user.role !== "admin") {
    throw new Error("You must be an admin to see Pending Asset");
  }

  try {
    const pendingAssets = await db.select({asset:asset,
        userName : user.name,
    }).from(asset)
