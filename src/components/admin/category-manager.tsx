...
const handleDeleteCategory = async(currenCategoryIdToDelete : number)=> {
    const result = await deleteCategoryAction(currenCategoryIdToDelete);

    if(result.success){
        setCategories(
            categories.filter((c) => c.id !== currenCategoryIdToDelete)
        )
    }
}
